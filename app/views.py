...
from .permissions import IsActiveOrder, IsOwnerProfile
from .serializers import LatestOrdersSerializer, OrderSerializer, ProfileSerializer
...
class LatestOrdersListAPIView(ListAPIView):
    ...
    * Only authenticated users can perform any action.
    ...
    def get_queryset(self):
        queryset = Order.objects.filter(status=True).exclude(
